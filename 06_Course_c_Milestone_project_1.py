...
# TEST Step 1: run your function on a test version of the board
...
player_input()
# Step 3: Write a function that takes in the 
# board list object, a marker ('X' or 'O'), and a desired position (number 1-9)
...
# using test parameters and display the modified board

place_marker(test_board,'T',8)
display_board(test_board)
# Step 4: Write a function that takes in a board
# and a mark (X or O) and then checks to see if that mark has won.
...
def choose_first():
    if random.randint(0, 1) == 0:
        ...
        return 'Player 1'
# Step 6: Write a function that returns a
# boolean indicating whether a space on the board is freely available.

def space_check(board, position):
   return board[position] == ' '
# Step 7: Write a function that checks if the board
# is full and returns a boolean value. True if full, False otherwise.
...
def full_board_check(board):
    ...
    for i in range(1,10):
        if space_check(board, i):
            return False
    return True
# Step 8: Write a function that asks for a 
# player's next position (as a number 1-9) and then uses the
